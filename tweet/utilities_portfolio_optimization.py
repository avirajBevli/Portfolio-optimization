...
def MV2(assets): #assets is a list of dict
	...
	return portfolio, 10000*tot_return, tot_risk

def convert_to_amount_from_weights(assets):
	portfolio = []
	from asset.models import Asset
	for asset_curr in assets:
		db_asset = Asset.objects.get(security_id=asset_curr['asset'])
		dict_temp = {
			#'asset':asset_curr['asset'],
			'asset':db_asset.ticker_name,
			'amount':asset_curr['weight']*10000,
		}
		portfolio.append(dict_temp)
	return portfolio
...
def calculate_portfolio(assets):
	...
	portfolio, exp_return, exp_risk  = MV2(our_assets)
	portfolio = convert_to_amount_from_weights(portfolio)
	return portfolio, exp_return, exp_risk